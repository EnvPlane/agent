...
`ENVPILOT_*` names during the migration window. The canonical name is derived
...
- Kubernetes selectors and `envpilot.io/*` labels/annotations used to find
  existing workloads;
- project namespace/resource naming (`envpilot-pr-*`) and filesystem defaults.
